 ...
 * Responsibilities
 * ─────────────────
 *  1. ROI map: load and manage per-parking spot regions of interest.
 *  2. Frame ingestion: receive grayscale frames from an upstream source
 *     (video recording in this case) via a lock-protected ring buffer.
 *  3. Occupancy analysis: frame-differencing + ROI pixel counting +
 *     shadow suppression + debounce to produce reliable spot events.
 *  4. IPC publish: send vp_event_t messages to the Parking Manager via
 *     QNX MsgSend.
 *
 * Threading model
 * ────────────────
 *  ingest_thread  – HIGH priority   – enqueues frames from source
 *  analysis_thread– MEDIUM priority – dequeues frames, runs CV logic
 */
                          ...
                          uint32_t w, uint32_t h)
{
    ...
    /* Clockwise vertices: TL, TR, BR, BL */
...
/*
 * Minimal JSON parser for ROI configuration file.
 * Expected format
 * {
 *   "spots": [
 *     { "id": 0, "label": "A-01",
...
/* Reference frame */
/*Load a pre-captured empty-lot reference from a raw grayscale file.*/
                       ...
                       uint32_t width, uint32_t height)
{
    ...
    if ((size_t)sz != nbytes) {
        vp_log("ERROR: reference file size mismatch: expected %zu bytes "
               "(%ux%u) but got %ld", nbytes, width, height, sz);
        fclose(f);
        return -1;
    }
    ...
    /* Initialise all spot contexts to UNKNOWN (will be resolved on first frame) */
...
/*
 * Heuristic shadow suppression: pixels in a narrow luminance band are
 * likely shadow artefacts on an otherwise static background.
 * Returns true if the pixel should be excluded from the diff count.
 */
...
/* For a given ROI, count pixels where |frame - reference| > threshold
 * (excluding shadow pixels), then divide by total ROI pixel count.
*/
...
/* IPC – SEND EVENT TO PARKING MANAGER */

/*
 * Transmit an occupancy event to the Parking Manager via QNX MsgSend.
*/

static int vp_send_event(vp_ctx_t *ctx, const vp_event_t *evt)
{
    uint32_t reply = 0;
    int rc = MsgSend(ctx->pm_coid,
                     evt,  sizeof(*evt),
                     &reply, sizeof(reply));
    if (rc == -1) {
...
/*
 * vp_enqueue_frame
 * Called by the ingestion thread (or camera callback).
...
/* Dequeue – called by analysis thread only */
...
static void *analysis_thread_fn(void *arg)
{
    ...
    while (ctx->running) {
        vp_frame_t frame;
        if (fq_dequeue(ctx, &frame) != 0)
            break;

        uint64_t t_start = vp_now_ns();

        if (!ctx->ref_valid) {
            size_t nbytes = frame.width * frame.height;
            if (!ctx->reference_frame) {
            ...
            vp_log("analysis_thread: reference frame set (frame %u, %ux%u); startup states remain UNKNOWN until a real transition is observed",
        ...
        for (uint32_t s = 0; s < ctx->roi_map.n_spots; s++) {
            ...
            float ratio = vp_compute_change_ratio(ctx, roi,
                                                   frame.pixels,
                                                   frame.width,
                                                   frame.height);

            vp_spot_state_t detected = vp_classify_spot(ratio,
                                            ctx->occupancy_ratio);

            /* ---- Debounce logic ---- */
            if (detected == sctx->candidate) {
                sctx->debounce_count++;
            } else {
            ...
            if (sctx->debounce_count >= ctx->debounce_frames &&
                detected != sctx->state)
            {
                /* State confirmed – build and send event */
                vp_event_t evt;
                ...
                vp_log("SPOT %s [%u]: %s -> %s (ratio=%.3f, frame=%u)",
                       evt.label, evt.spot_id,
                       (evt.old_state == VP_SPOT_FREE ? "FREE" :
                        evt.old_state == VP_SPOT_OCCUPIED ? "OCCUPIED" : "UNKNOWN"),
                       (evt.new_state == VP_SPOT_FREE ? "FREE" :
                        evt.new_state == VP_SPOT_OCCUPIED ? "OCCUPIED" : "UNKNOWN"),
                       ratio, frame.frame_id);

                if (vp_send_event(ctx, &evt) == 0)
                    ctx->events_sent++;
            }
        }
        ...
        if (latency > ctx->max_latency_ns)
            ctx->max_latency_ns = latency;
        ctx->frames_processed++;

        free(frame.pixels); /* analysis is done with this frame */
    }
    ...
    vp_log("analysis_thread: exiting (frames=%llu events=%llu avg_lat=%llu ns max_lat=%llu ns)",
           ...
           ctx->frames_processed
               ? (unsigned long long)(ctx->total_latency_ns / ctx->frames_processed)
               : 0ULL,
...
int vp_init(vp_ctx_t *ctx, const vp_roi_map_t *roi_map, int pm_coid)
{
    ...
    /* Initialise all spot contexts to UNKNOWN */
    for (uint32_t i = 0; i < VP_MAX_SPOTS; i++) {
        ctx->spot_ctx[i].state     = VP_SPOT_UNKNOWN;
        ctx->spot_ctx[i].candidate = VP_SPOT_UNKNOWN;
    }

    if (pthread_mutex_init(&ctx->fq_mutex, NULL) != 0) {
        vp_log("ERROR: fq_mutex init failed");
        return -1;
    }
    if (pthread_cond_init(&ctx->fq_not_empty, NULL) != 0) {
        pthread_mutex_destroy(&ctx->fq_mutex);
        return -1;
    }
    if (pthread_cond_init(&ctx->fq_not_full, NULL) != 0) {
        pthread_mutex_destroy(&ctx->fq_mutex);
        pthread_cond_destroy(&ctx->fq_not_empty);
        return -1;
    }

    vp_log("VP init: %u spots, diff_thr=%u, occ_ratio=%.2f, debounce=%u",
           ctx->roi_map.n_spots,
           ctx->diff_threshold,
           ctx->occupancy_ratio,
           ctx->debounce_frames);
...
int vp_start(vp_ctx_t *ctx)
{
    ...
    sp.sched_priority = 20; /* medium-high – below Parking Manager */
    pthread_attr_setschedparam(&attr, &sp);
    pthread_attr_setinheritsched(&attr, PTHREAD_EXPLICIT_SCHED);

    if (pthread_create(&ctx->analysis_thread, &attr, analysis_thread_fn, ctx) != 0) {
        vp_log("ERROR: failed to create analysis_thread: %s", strerror(errno));
        ctx->running = false;
...
void vp_stop(vp_ctx_t *ctx)
{
    ...
    /* Wake threads blocked on condition variables */
